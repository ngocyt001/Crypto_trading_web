...
tabs.forEach((tab, index) => {
    ...
    tab.onclick = function () {
        ...
        $$('.content-list-item.active').classList.remove('active')
...
var taskObj = {
    ...
    deleteProject: function (self) {
        ...
        swal({
            title: "Are you sure?",
            text: "Deleting the project will delete its data too.",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        })
        ...
        for (var a = 0; a < projects.length; a++) {
            ...
            for (var b = 0; b < projects[a].tasks.length; b++) {
                html += "<tr>";
                    ...
                    var coin_compare = projects[a].name;
                    var coin_price_compare = projects[a].tasks[b].name;
                    $.ajax(settings).done(function(response) {
                        ...
                        var dataPrice_temp = `dataPrice_temp = response.${coin_compare}.usd`;
                        eval(dataPrice_temp); // thực thi lệnh từ string
                    
                        if(coin_price_compare==dataPrice_temp) {
                            swal({
                                title: `Hmm, ${coin_compare} Meet Target`,
                                text: `Check your watching list, ${coin_compare} matched target`,
                                icon: "warning",
                                buttons: true,
                                dangerMode: true,
                              })
                              .then((willDelete) => {
                                if (willDelete) {
                                  swal("Success! Hope you win that game!", {
                                    icon: "success",
                                  });
                                } else {
                                  swal("Tracking next coin to reach the change!");
                                }
                              });
                        }
                        // console.log(coin_price_compare)
                
                    })
                    // console.log(projects[a].tasks[b].name)
                ...
                for (var b = 0; b < projects[a].tasks.length; b++) {
                    ...
                    if (projects[a].tasks[b].id == form.task.value) {
                        ...
                        if (self.value == "delete") {
                            ...
                            swal({
                                title: "Are you sure?",
                                text: "Deleting the target will delete its data too, you may miss change.",
                                icon: "warning",
                                buttons: true,
                                dangerMode: true,
                            })
...
window.addEventListener("load", function () {
    ...
    var test = document.querySelector('#all-tasks td').firstChild;
    console.log(test)
